...
double IIR_LPF_LM35(double data);            // IIR filter(fc=3) 함수 선언_LM35에 사용
double IIR_LPF_PM(double data);              // IIR filter(fc=8) 함수 선언_가변저항에 사용
double IIR_LPF_IR(double data);              // IIR filter(fc=8) 함수 선언_IR에 사용
double FIR_LPF(double data);                 // FIR filter(fc=5) 함수 선언_CDS 에사용
double IIR_LPF(double data);                 // IIR filter(fc=5) 함수 선언_Thermister에 사용
...
volatile double b_iir_LM35[3]={0.00008766,0.00017531,0.00008766};             // iir b계수
volatile double a_iir_LM35[3]={1.0,-1.97334425,0.97369487};                  // iir a계수
volatile double IIR_input_LM35[3]={0,};                                    // 이전adc값 저장
volatile double IIR_LM35[3]={0,};                                          // 출력값 저장
...
int main(void)
{
   ...
   TCCR1A = (1<<COM1A1)|(0<<COM1A0)|(1<<COM1B1)|(0<<COM1B0)|(1<<COM1C1)|(0<<COM1C0)|(1<<WGM11)|(0<<WGM10); // Fast PWM Mode
...
double IIR_LPF_LM35(double data)
{
	double iir_save_LM35=0; // 이전adc값 저장
	int j=0;
	int k=0;
	IIR_input_LM35[0] = data;
	IIR_LM35[0] = 0;
	
	for(j=0;j<3;j++) IIR_LM35[0] += b_iir_LM35[j]*IIR_input_LM35[j]; // IIR 이전 ADC값에 b계수 곱하여 더해주기
	for(k=1;k<3;k++) IIR_LM35[0] -= a_iir_LM35[k]*IIR_LM35[k];       // IIR 이전 출력값에 a계수 곱하여 빼주기
	
	iir_save_LM35=IIR_LM35[1];                    /////////////////////////
	IIR_LM35[1]=IIR_LM35[0];                      /////////////////////////
	IIR_LM35[2]=iir_save_LM35;                    //이전adc값과 출력값 저장//
	iir_save_LM35=IIR_input_LM35[1];              /////////////////////////
	IIR_input_LM35[1]=IIR_input_LM35[0];          /////////////////////////
	IIR_input_LM35[2]=iir_save_LM35;              /////////////////////////
